...
# omitting NAs leads to unpredictable vector length. Easy to in this case, but for now...
options(na.action="na.pass")

# # patched version of svymean to get around smaller issues with matrix default values and class()
...
#   }
#     return(rval)
#     # THIS SOMEHOW FORCES COMPUTATION, IGNORE
# #  class(rval)<-"svrepstat"
# }


svymean_p<-svrepmean<-function(x,design, na.rm=FALSE, rho=NULL,
                                           return.replicates=FALSE,deff=FALSE,...)
{
...
system.time({
  print(svymean_p(~agep, svydsgn, se=TRUE))
  
  })
system.time({
  print(svymean_p(~relp, svydsgn, se=TRUE))
